...
import com.mall.common.utils.VerifyCodeUtils;
import com.mall.core.shiro.token.manager.TokenManager;
import com.mall.core.statics.Constant;
import com.mall.user.manager.UserManager;
...
public class UserLoginController extends BaseController {
	...
	public Map<String,Object> subRegister(String vcode,UUser entity){
		...
		entity.setLoginType(Constant.LOGIN_UNAME);
	...
	@ResponseBody
	public Map<String,Object> submitLogin(UUser entity,String loginname,Boolean rememberMe,HttpServletRequest request){
		 //判断是邮箱还是手机号的正则表达式  
        String em = "^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$";  
        String ph = "^[1][34578]\\d{9}$";  
		try {
			if(entity.getUsername().matches(ph)) {
				entity.setLoginType(Constant.LOGIN_PHONE);
			}else if(entity.getUsername().matches(em)) {
				entity.setLoginType(Constant.LOGIN_EMAIL);
			}else {
				entity.setLoginType(Constant.LOGIN_UNAME);
			}
